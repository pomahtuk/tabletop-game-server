...
const computerTestPlayerNormal = new ComputerPlayerNormal(
  "computer easy normal"
);
...
describe("playerMapper", (): void => {
  ...
  it("Able to map human player to human player with stats", (): void => {
    ...
    expect(humanCopy.id).toMatch(humanTestPlayer.id);
    expect(humanCopy.statEnemyFleetsDestroyed).toBe(
      humanTestPlayer.statEnemyFleetsDestroyed
    );
    expect(humanCopy.statShipCount).toBe(humanTestPlayer.statShipCount);
    expect(humanCopy.statEnemyShipsDestroyed).toBe(
      humanTestPlayer.statEnemyShipsDestroyed
    );
  });
  ...
  it("Able to map computer player easy to computer player easy without stats", (): void => {
    ...
    expect((computerCopy as ComputerPlayer).computerType).toBe(
  ...
  it("Able to map computer player normal to computer player normal without stats", (): void => {
    ...
    expect(computerCopy.screenName).toMatch(
      computerTestPlayerNormal.screenName
    );
    expect(computerCopy.isComputer).toBe(true);
    expect(computerCopy.id).toMatch(computerTestPlayerNormal.id);
    expect((computerCopy as ComputerPlayer).computerType).toBe(
      ComputerPlayerType.NORMAL
    );
  });
  ...
  it("Able to map computer player to computer player with stats", (): void => {
    ...
    expect(computerCopy.statEnemyFleetsDestroyed).toBe(
      computerTestPlayerEasy.statEnemyFleetsDestroyed
    );
    expect(computerCopy.statShipCount).toBe(
      computerTestPlayerEasy.statShipCount
    );
    expect(computerCopy.statEnemyShipsDestroyed).toBe(
      computerTestPlayerEasy.statEnemyShipsDestroyed
    );
  });
});
